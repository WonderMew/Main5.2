...
void CreateClassAttributes()
{
	...
	CreateClassAttribute( 6, 32, 27, 25, 20,	 100, 40,	 /*荤侩救窃 => */1, 3, 1, 1);
...
void CHARACTER_MACHINE::CalculateDamage()
{
	...
	if(g_isCharacterBuff((&Hero->Object), eBuff_BlessingOfXmax))	//农府胶付胶狼 绵汗
	{
		int _Temp = 0;
	...
	if(g_isCharacterBuff((&Hero->Object), eBuff_StrengthOfSanta))	//魂鸥狼 碍拳
	{
		int _Temp = 30;
...
void CHARACTER_MACHINE::CalculateAttackSpeed()
{
	...
	if( (false == Amulet->bPeriodItem || false == Amulet->bExpiredPeriod) && Amulet->Type == ITEM_HELPER + 114 )
	{
		...
		//Character.AttackSpeed += Amulet->WeaponSpeed;	// 辑滚俊辑 逞绢坷绰 蔼? 眠饶 犬牢荤亲.
